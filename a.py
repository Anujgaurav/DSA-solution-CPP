...
Label(root,text='IMAGE VIEWER IN PYTHON', font='fantesy 20 bold').pack(side=BOTTOM)
  ...
  def open_file(self):
  self.path = filedialog.askdirectory() if self.path:
  self.Image_list = [] 
  self.add_image()

def add_image(self):
for image in os.listdir(self.path):
ext = image.split('.')[::-1][0].upper() if ext in self.extension:
self.Image_list.append(image)

self.resize(self.Image_list[0]) def resize(self, image):
if self.path:
os.chdir(self.path)
image_p = self.path + '\\' + str(image) img = PIL.Image.open(image)
 
width, height = img.size
if (int(width) > 850 and int(height) < 500): img = img.resize((850, height))
elif (int(height) > 500 and int(width) < 850): img = img.resize((width, 500))
elif (int(width) > 850 and int(height) > 500): img = img.resize((700, 500))
...
if__name=__= ' main ': 
    root = Tk()
img = ImageView(root) root.configure(background="pink") 
root.mainloop()
